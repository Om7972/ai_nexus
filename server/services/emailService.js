...
export const sendVerificationEmail = async ({ name, email, token }) => {
  return emailService.sendVerificationEmail({ name, email }, token);
};

export const sendPasswordResetEmail = async ({ name, email, token }) => {
  return emailService.sendPasswordResetEmail({ name, email }, token);
};
...
export const sendPasswordChangedEmail = async ({ name, email }) => {
  const html = `
    ...
    <body>
      <div class="container">
        ...
        <div class="content">
          <p>Hi ${name},</p>
          <p>This is a confirmation that the password for your AI Nexus account has just been changed.</p>
  ...
  return emailService.sendEmail({
    to: email,
    subject: '🔐 Your AI Nexus Password Has Been Changed',
    html,
  });
};
